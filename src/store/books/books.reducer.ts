    ...
    extraReducers: (builder) => {
      builder
        ...
        .addCase(fetchBooks.rejected, (state, action) => {
          state.isLoading = false;
          state.errorFetchBooks = action.error.message ?? "Failed to get book list";
        })
        ...
        .addCase(updateBookOptimistic.rejected, (state, action) => {
          state.errorUpdateAction = action.error.message ?? "Failed to update book";
        })
        ...
        .addCase(deleteBookOptimistic.rejected, (state, action) => {
          state.errorUpdateAction = action.error.message ?? "Failed to delete book";
        });
    },
});
