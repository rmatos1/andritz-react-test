...
describe("<BookList />", () => {
  ...
  it("should display no books matched message when there are no books that match the search criteria", () => {
  ...
  it("should display the correct text for displaying books", () => {
    const wrapper = render(setup());

    const displayingText = wrapper.getByText(
      `Displaying ${defaultProps.books.length} of ${defaultProps.totalBooks} books`
    );
    expect(displayingText).toBeTruthy();
  });
  ...
  it("should display the correct number of books per page when modified", () => {
    ...
    fireEvent.change(selectElement, {
      target: { value: DEFAULT_BOOKS_PER_PAGE },
    });
